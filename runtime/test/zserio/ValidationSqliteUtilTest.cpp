...
protected:
    ...
    using TiedColumn = std::tuple<Util::StringType, Util::StringType, bool, bool>;
...
{
    ASSERT_THROW(Util::getNumberOfTableRows(connection, "", "test", allocator_type()), SqliteException);
    ...
    insertRows("test", 0, 1);
    ASSERT_EQ(1, Util::getNumberOfTableRows(connection, "", "test", allocator_type()));

    insertRows("test", 1, 10);
    ASSERT_EQ(11, Util::getNumberOfTableRows(connection, "", "test", allocator_type()));

    insertRows("test", 11, 10);
    ASSERT_EQ(21, Util::getNumberOfTableRows(connection, "", "test", allocator_type()));
}

TEST_F(ValidationSqliteUtilTest, getTableSchema)
{
    Util::TableSchema schema;
    Util::getTableSchema(connection, "", "test", schema, allocator_type());
    ASSERT_TRUE(schema.empty());

    ASSERT_THROW(
            Util::getTableSchema(connection, "NONEXISTING", "test", schema, allocator_type()), SqliteException);

    connection.executeUpdate("CREATE TABLE test1(id INTEGER PRIMARY KEY NOT NULL, value INTEGER NOT NULL)");
    Util::getTableSchema(connection, "", "test1", schema, allocator_type());
    ...
    connection.executeUpdate(
            ...
            "field BLOB)");
    Util::getTableSchema(connection, "", "test2", schema, allocator_type());
    ...
    Util::getTableSchema(connection, "", "test3", schema, allocator_type());
    ASSERT_EQ(3, schema.size());
...
{
    ASSERT_FALSE(Util::isColumnInTable(connection, "", "test", "hidden", allocator_type()));

    ASSERT_FALSE(Util::isColumnInTable(connection, "NONEXISTING", "test", "hidden", allocator_type()));

    connection.executeUpdate("CREATE TABLE test1(id INTEGER PRIMARY KEY NOT NULL, value INTEGER NOT NULL)");
    ASSERT_FALSE(Util::isColumnInTable(connection, "", "test1", "hidden", allocator_type()));
    ASSERT_TRUE(Util::isColumnInTable(connection, "", "test1", "id", allocator_type()));
    ASSERT_TRUE(Util::isColumnInTable(connection, "", "test1", "value", allocator_type()));

    connection.executeUpdate("CREATE TABLE test2(id INTEGER PRIMARY KEY NOT NULL, text HIDDEN TEXT)");
    ASSERT_TRUE(Util::isColumnInTable(connection, "", "test2", "id", allocator_type()));
    ASSERT_TRUE(Util::isColumnInTable(connection, "", "test2", "text", allocator_type()));
}
