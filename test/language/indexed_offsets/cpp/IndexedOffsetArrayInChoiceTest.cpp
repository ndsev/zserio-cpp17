...
#include "indexed_offsets/indexed_offset_array_in_choice/TestStruct.h"
#include "test_utils/TestUtility.h"

namespace indexed_offsets
{
namespace indexed_offset_array_in_choice
{

class IndexedOffsetArrayInChoiceTest : public ::testing::Test
{
protected:
    ...
    static void checkOffsets(const zserio::View<TestStruct>& view)
    {
        for (uint32_t i = 0; i < SIZE; ++i)
        {
            ASSERT_EQ(FIRST_OFFSET + i * 4, view.choiceField().offsetArray()[i]);
        }
    }
...
TEST_F(IndexedOffsetArrayInChoiceTest, read)
{
    TestStruct data;
    fillData(data, true);
...
TEST_F(IndexedOffsetArrayInChoiceTest, writeRead)
{
    TestStruct data;
    fillData(data);
    zserio::View view(data);

    for (auto offset : view.choiceField().offsetArray())
    {
        EXPECT_EQ(0, offset);
    }
