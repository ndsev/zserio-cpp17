...
{
    constexpr float diff = []() -> float {
        if constexpr (3.13F - FLOAT16_CONSTANT < 0.0F)
        {
            return FLOAT16_CONSTANT - 3.13F;
        }
        else
        {
            return 3.13F - FLOAT16_CONSTANT;
        }
    }();
...
{
    constexpr float diff = []() -> float {
        if constexpr (3.131F - FLOAT32_CONSTANT < 0.0F)
        {
            return FLOAT32_CONSTANT - 3.131F;
        }
        else
        {
            return 3.131F - FLOAT32_CONSTANT;
        }
    }();
    static_assert(diff <= std::numeric_limits<float>::epsilon(), "shall be constexpr");
...
{
    constexpr double diff = []() -> double {
        if constexpr (3.1314 - FLOAT64_CONSTANT < 0.0)
        {
            return FLOAT64_CONSTANT - 3.1314;
        }
        else
        {
            return 3.1314 - FLOAT64_CONSTANT;
        }
    }();
...
TEST(ConstantsTest, stringConstant)
{
    ...
    ASSERT_EQ("Test \"Quated\" String"sv, STRING_CONSTANT);
}
...
TEST(ConstantsTest, joinedStringConstant)
{
    ...
    ASSERT_EQ("This is: Test \"Quated\" String"sv, JOINED_STRING_CONSTANT);
}
...
TEST(ConstantsTest, unicodeEscStringConstant)
{
    ...
    ASSERT_EQ("Test string with unicode escape \x19"sv, UNICODE_ESC_STRING_CONSTANT);
}
...
TEST(ConstantsTest, hexEscStringConstant)
{
    ...
    ASSERT_EQ("Test string with hexadecimal escape \x19"sv, HEX_ESC_STRING_CONSTANT);
}
...
TEST(ConstantsTest, octalEscStringConstant)
{
    ...
    ASSERT_EQ("Test string with octal escape \031"sv, OCTAL_ESC_STRING_CONSTANT);
}
...
TEST(ConstantsTest, stringPascalCaseConstant)
{
    ...
    ASSERT_EQ("Different naming convention"sv, StringPascalCaseConstant);
}
