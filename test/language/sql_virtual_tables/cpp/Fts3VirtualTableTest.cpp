...
using AllocatorType = Fts3TestDb::allocator_type;
...
protected:
    static void fillRow(Fts3VirtualTable::Row& row, const StringType& title, const StringType& body)
    {
        row.title = title;
        row.body = body;
    }

    static void fillRows(VectorType<Fts3VirtualTable::Row>& rows)
    {
        rows.clear();
    ...
    static void checkRow(const Fts3VirtualTable::Row& row1, const Fts3VirtualTable::Row& row2)
    {
