...
protected:
    ...
    void insertRowsWithBlob(sqlite3_stmt* statement, uint32_t numRows)
    {
        for (uint32_t id = 0; id < numRows; ++id)
        {
            ...
            auto bytesBuffer = zserio::serializeToBytes(blob);
            sqlite3_bind_blob(statement, argIdx++, bytesBuffer.data(), static_cast<int>(bytesBuffer.size()),
                    SQLITE_TRANSIENT);
    ...
    void writeInvalidBlobToTable2(zserio::SqliteConnection& connection, uint32_t id)
    {
        ...
        writer.writeUnsignedBits32(0xff, 8);
...
private:
    static constexpr const char* DB_FILE_NAME = "arguments/with_validation_code/control_validation_test.sqlite";
};
