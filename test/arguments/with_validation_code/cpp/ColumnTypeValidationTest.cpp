...
protected:
    void populateDb()
    {
        ...
        for (size_t i = 1; i <= ENTRY_COUNT; ++i)
        {
            ...
            row.id = static_cast<int8_t>(i);
            row.int8Value = static_cast<int8_t>(-1 * static_cast<int8_t>(i));
            row.int16Value = static_cast<int16_t>(-2 * static_cast<int8_t>(i));
            row.int32Value = -3 * static_cast<int8_t>(i);
            row.int64Value = -4 * static_cast<int8_t>(i);
            row.uint8Value = 1 * static_cast<uint8_t>(i);
            row.uint16Value = static_cast<uint16_t>(2 * static_cast<uint16_t>(i));
            row.uint32Value = 3 * static_cast<uint32_t>(i);
            row.uint64Value = 4 * i;
            row.float16Value = 1.1F * static_cast<float>(i);
            row.float32Value = 1.2F * static_cast<float>(i);
            row.float64Value = 1.3 * static_cast<double>(i);
            row.stringValue = "stringValue" + zserio::toString(i, m_database->get_allocator());
        }
