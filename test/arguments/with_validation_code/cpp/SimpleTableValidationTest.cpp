...
protected:
    ...
    {
    public:
        zserio::UInt32 localCount1(const zserio::View<SimpleTable::Row>&) override
        {
...
private:
    void insertTestRow(zserio::SqliteConnection& connection, uint32_t id, bool wrongCount)
    {
        ...
        RootStruct fieldBlob = createTestRootStruct(wrongCount ? 1 : SIMPLE_TABLE_LOCAL_COUNT, id);
    ...
    void insertRow(zserio::SqliteConnection& connection, uint32_t id, bool fieldBool, uint8_t fieldBit5,
            ...
            const RootStruct& rootStruct, TestEnum fieldEnum, TestBitmask fieldBitmask)
    {
        ...
        auto byteBuffer = zserio::serializeToBytes(rootStruct, static_cast<uint32_t>(rootStruct.filler.size()));
        uint8_t* buffer = byteBuffer.data();
        sqlite3_bind_blob(
                statement.get(), argIdx++, buffer, static_cast<int>(byteBuffer.size()), SQLITE_TRANSIENT);
