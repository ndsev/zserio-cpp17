...
public:
    ...
    {
        if (m_reflectable && m_storage.getBitSize() == 0)
        {
            // lazy initialization
            m_storage = BasicBitBuffer<ALLOC>(m_reflectable->bitSizeOf(), m_storage.get_allocator());
            BitStreamWriter writer(m_storage);
...
private:
    ...
    mutable BasicBitBuffer<ALLOC> m_storage;
};
...
public:
    ...
    explicit BasicObjectServiceData(ZSERIO_OBJECT& object, const ALLOC& allocator = ALLOC()) :
            m_storage(object.bitSizeOf(), allocator)
    {
        BitStreamWriter writer(m_storage);
        object.write(writer);
    }
...
private:
    BasicBitBuffer<ALLOC> m_storage;
};
...
private:
    vector<uint8_t, ALLOC> m_storage;
};
